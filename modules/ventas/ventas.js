...
(function(){
  ...
  const elCashStatus = $("v-cashStatus");
  ...
  function fmtCashTime(at){
    if(!at) return "—";
    const d = (window.dpParseAtLocal ? window.dpParseAtLocal(at) : new Date(at));
    if(!d || isNaN(d.getTime())) return String(at);
  ...
  function canSell(){
    ...
    if(!activeCash) return { ok:false, msg:"No hay caja abierta." };
