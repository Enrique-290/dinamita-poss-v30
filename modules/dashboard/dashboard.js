...
(function(){
  ...
  function refresh(){
    ...
    const activeCash = (typeof dpGetActiveCashSession === "function") ? dpGetActiveCashSession() : null;
    const cashSessions = (typeof dpGetCashSessions === "function") ? dpGetCashSessions() : [];
    const elCash = $("#dp-kpi-cash");
    const elCashHint = $("#dp-kpi-cash-hint");
    if(elCash) elCash.textContent = activeCash ? money(activeCash?.totals?.total || 0) : "Cerrada";
    if(elCashHint) elCashHint.textContent = activeCash ? `${activeCash.userName || 'Usuario'} · ${activeCash.openedAt?.slice(11,16) || ''}` : "Sin caja abierta";

    const cashByPayment = activeCash?.totals?.byPayment || {};
    const cashCurrentRows = activeCash ? [
      { title: 'Usuario', sub: activeCash.userName || 'Usuario', right: activeCash.openedAt?.slice(11,16) || '' },
      { title: 'Fondo inicial', sub: 'Monto de apertura', right: money(activeCash.openingAmount || 0) },
      { title: 'Ventas de productos', sub: 'Ventas ligadas al corte', right: money(activeCash?.totals?.sales || 0) },
      { title: 'Membresías', sub: 'Cobros de membresía', right: money(activeCash?.totals?.memberships || 0) },
      { title: 'Servicios', sub: 'Otros cobros', right: money(activeCash?.totals?.services || 0) },
      { title: 'Efectivo', sub: 'Cobrado en efectivo', right: money(cashByPayment.efectivo || 0) },
      { title: 'Tarjeta', sub: 'Cobrado con tarjeta', right: money(cashByPayment.tarjeta || 0) },
      { title: 'Transferencia', sub: 'Cobrado por transferencia', right: money(cashByPayment.transferencia || 0) },
      { title: 'Otro', sub: 'Otros métodos de pago', right: money(cashByPayment.otro || 0) },
      { title: 'Total del corte', sub: 'Venta acumulada actual', right: money(activeCash?.totals?.total || 0) },
    ] : [];
    renderRows($("#dp-cash-current"), cashCurrentRows);
    const cashCurrentSub = $("#dp-cash-current-sub");
    if(cashCurrentSub) cashCurrentSub.textContent = activeCash ? `Abierta desde ${activeCash.openedAt || ''}` : 'No hay caja abierta';

    const lastCuts = cashSessions.slice().sort((a,b)=> String(b.openedAt||'').localeCompare(String(a.openedAt||''))).slice(0,6);
    const lastCutRows = lastCuts.map(c=>({
      title: `${c.userName || 'Usuario'} · ${c.status === 'open' ? 'Abierta' : 'Cerrada'}`,
      sub: `Efe: ${money(c?.totals?.byPayment?.efectivo || 0)} · Tar: ${money(c?.totals?.byPayment?.tarjeta || 0)} · Transf: ${money(c?.totals?.byPayment?.transferencia || 0)}`,
      right: money(c?.totals?.total || 0)
    }));
    renderRows($("#dp-cash-lastcuts"), lastCutRows);
    const lastCutsSub = $("#dp-cash-lastcuts-sub");
    if(lastCutsSub){
      const opened = cashSessions.filter(c=>c.status==='open').length;
      const closed = cashSessions.filter(c=>c.status==='closed').length;
      lastCutsSub.textContent = `Abiertas: ${opened} · Cerradas: ${closed}`;
    }
