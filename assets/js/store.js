...
function dpDefaultState(){
  return {
    ...
    users: [
      { id:'U001', name:'Administrador', username:'admin', password:'1234', role:'admin', active:true, createdAt:dpNowISO() }
    ],
    cashControl: {
      activeSessionId: "",
      sessions: []
...
function dpEnsureCashState(st){
  st.cashControl = st.cashControl || { activeSessionId:"", sessions:[] };
  st.cashControl.sessions = Array.isArray(st.cashControl.sessions) ? st.cashControl.sessions : [];
  return st.cashControl;
}

function dpGetCashSessions(){
  const st = dpGetState();
  const cc = dpEnsureCashState(st);
  return cc.sessions.slice();
}

function dpGetActiveCashSession(){
  const st = dpGetState();
  const cc = dpEnsureCashState(st);
  if(!cc.activeSessionId) return null;
  return cc.sessions.find(x=>x.id===cc.activeSessionId && x.status==="open") || null;
}

function dpOpenCashSession({ openingAmount=0 }={}){
  return dpSetState(st=>{
    const cc = dpEnsureCashState(st);
    const currentUser = dpGetCurrentUser();
    ...
    const session = {
      ...
      notes: "",
      status: "open",
      totals: { sales:0, memberships:0, services:0, total:0, byPayment:{ efectivo:0, tarjeta:0, transferencia:0, otro:0 } }
    };
    cc.activeSessionId = session.id;
    cc.sessions.unshift(session);
    return st;
  });
}

function dpCloseCashSession({ countedAmount=0, notes="" }={}){
  return dpSetState(st=>{
    const cc = dpEnsureCashState(st);
    const active = cc.sessions.find(x=>x.id===cc.activeSessionId && x.status==="open");
...
function dpTouchCashSessionWithSale(st, sale){
  const cc = dpEnsureCashState(st);
  const active = cc.sessions.find(x=>x.id===cc.activeSessionId && x.status==="open");
  if(!active) throw new Error("No hay caja abierta. Abre caja para vender.");
  sale.cashSessionId = active.id;
  sale.cashSessionUserId = active.userId || "";
  sale.cashSessionUserName = active.userName || "";
  const total = Number(sale.total||0);
  active.totals = active.totals || { sales:0, memberships:0, services:0, total:0, byPayment:{ efectivo:0, tarjeta:0, transferencia:0, otro:0 } };
  active.totals.byPayment = active.totals.byPayment || { efectivo:0, tarjeta:0, transferencia:0, otro:0 };
  active.totals.total += total;
  const pm = String(sale.paymentMethod || "efectivo").toLowerCase();
  if(active.totals.byPayment[pm] == null) active.totals.byPayment[pm] = 0;
  active.totals.byPayment[pm] += total;
  if(sale.type === "venta") active.totals.sales += total;
  else if(sale.meta && sale.meta.kind === "membership") active.totals.memberships += total;
  else active.totals.services += total;
}
...
function dpCreateSale({clientId, cartItems, note, iva=0, paymentMethod="efectivo"}){
  return dpSetState(st => {
    ...
    dpTouchCashSessionWithSale(st, saleRecord);
    st.sales.unshift(saleRecord);
...
function dpEnsureSeedData(){
  return dpSetState(st=>{
    ...
    if(st.users.length===0){
      st.users = [{ id:'U001', name:'Administrador', username:'admin', password:'1234', role:'admin', active:true, createdAt:dpNowISO() }];
    }
...
function dpDeleteSale(ticketId){
  return dpSetState(st=>{
    ...
    // Reverse totals in cash session if linked
    if(sale.cashSessionId){
      const cc = dpEnsureCashState(st);
      const sess = (cc.sessions||[]).find(x=>x.id===sale.cashSessionId);
      if(sess && sess.totals){
        const total = Number(sale.total||0);
        sess.totals.total = Math.max(0, Number(sess.totals.total||0) - total);
        const pm = String(sale.paymentMethod||"efectivo").toLowerCase();
        sess.totals.byPayment = sess.totals.byPayment || {};
        sess.totals.byPayment[pm] = Math.max(0, Number(sess.totals.byPayment[pm]||0) - total);
        if(sale.type === "venta") sess.totals.sales = Math.max(0, Number(sess.totals.sales||0) - total);
        else if(sale.meta && sale.meta.kind === "membership") sess.totals.memberships = Math.max(0, Number(sess.totals.memberships||0) - total);
        else sess.totals.services = Math.max(0, Number(sess.totals.services||0) - total);
      }
    }
...
const DP_SESSION_KEY = "dp_v0_session_user";
...
function dpSaveUser(user){
  return dpSetState(st=>{
    st.users = Array.isArray(st.users) ? st.users : [];
    const clean = {
      id: user.id || dpId("U"),
      name: String(user.name || "").trim() || "Usuario",
      username: String(user.username || "").trim().toLowerCase(),
      password: String(user.password || "1234"),
      role: String(user.role || "empleado").toLowerCase()==="admin" ? "admin" : "empleado",
      active: user.active !== false,
      createdAt: user.createdAt || dpNowISO()
    };
    const idx = st.users.findIndex(x=>x.id===clean.id);
    if(idx>=0){
      st.users[idx] = { ...st.users[idx], ...clean };
    }else{
      st.users.unshift(clean);
    }
    return st;
  });
}

function dpDeleteUser(userId){
  return dpSetState(st=>{
    st.users = Array.isArray(st.users) ? st.users : [];
    const target = st.users.find(x=>x.id===userId);
    if(!target) return st;
    const admins = st.users.filter(x=>x.active!==false && x.role==="admin" && x.id!==userId);
    if(target.role==="admin" && admins.length===0){
      throw new Error("Debe existir al menos un administrador activo.");
    }
    st.users = st.users.filter(x=>x.id!==userId);
    return st;
  });
}

function dpSetUserActive(userId, active){
  return dpSetState(st=>{
    st.users = Array.isArray(st.users) ? st.users : [];
    const target = st.users.find(x=>x.id===userId);
    if(!target) return st;
    if(target.role==="admin" && active===false){
      const otherAdmins = st.users.filter(x=>x.id!==userId && x.active!==false && x.role==="admin");
      if(otherAdmins.length===0) throw new Error("Debe existir al menos un administrador activo.");
    }
...
function dpGetCurrentUser(){
  try{
    const raw = sessionStorage.getItem(DP_SESSION_KEY);
    if(!raw) return null;
    const sess = JSON.parse(raw);
    const user = dpGetUsers().find(x=>x.id===sess.id && x.active!==false);
    return user || null;
  }catch(e){ return null; }
}

function dpLogin(username, password){
  const uname = String(username||"").trim().toLowerCase();
  const pass = String(password||"");
  const user = dpGetUsers().find(x=>String(x.username||"").toLowerCase()===uname && String(x.password||"")===pass && x.active!==false);
  if(!user) return { ok:false, message:"Usuario o contraseña incorrectos." };
  try{ sessionStorage.setItem(DP_SESSION_KEY, JSON.stringify({ id:user.id, at:dpNowISO() })); }catch(e){}
  return { ok:true, user };
}

function dpLogout(){
  try{ sessionStorage.removeItem(DP_SESSION_KEY); }catch(e){}
}

function dpIsAdmin(){
  const u = dpGetCurrentUser();
...
function dpRenderBranding(){
  ...
  const appName = String(biz.name || "Dinamita POS").trim() || "Dinamita POS";
  if(nameEl) nameEl.textContent = appName;
  if(document && document.title) document.title = appName;
  ...
  if(img){
    if(biz.logoDataUrl){
      ...
      if(fb){
        ...
        const initials = appName.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("").toUpperCase();
        fb.textContent = initials || "DG";
      }
    }
  }

  document.querySelectorAll('[data-biz-name]').forEach(el=>{ el.textContent = appName; });
  document.querySelectorAll('[data-biz-logo]').forEach(el=>{
    if(!biz.logoDataUrl){
      if(el.tagName === 'IMG'){
    ...
    if(el.tagName === 'IMG'){
      el.src = biz.logoDataUrl;
      el.style.display = 'block';
    }else{
      el.style.backgroundImage = `url(${biz.logoDataUrl})`;
    }
  });
}
